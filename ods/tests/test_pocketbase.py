...
def compose_env(tmp_path):
    ...
    env.update(POCKETBASE_PASSWORD=secrets.token_hex(24), POCKETBASE_ENCRYPTION_KEY=secrets.token_hex(16), BIND_ADDRESS="127.0.0.1")
    return env
...
def test_bind_address_opt_in_preserves_native_configuration(tmp_path, compose_env, bind_address):
    ...
    baseline = json.loads(render(tmp_path, compose_env).stdout)["services"]["pocketbase"]
