"""The MLflow recipe must survive the real install and re-enable boundaries."""
...
def test_mlflow_install_and_reenable_use_an_image_only_compose(test_client, monkeypatch, tmp_path):
    ...
    def accept_install(service_id):
        ...
        assert (installed / "requirements-auth.txt").read_bytes() == (library / "mlflow/requirements-auth.txt").read_bytes()
    ...
    response = test_client.post("/api/extensions/mlflow/install", headers=test_client.auth_headers)
    assert response.status_code == 200, response.text
    assert response.json()["restart_required"] is False
    agent.assert_called_once_with("mlflow")
    ...
    for action in ("disable?include_data_info=false", "enable"):
        response = test_client.post(f"/api/extensions/mlflow/{action}", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
