"""The authenticated install must materialize the Markdown definition before host startup."""
...
def test_silverbullet_http_install_preserves_authentication_and_prepares_nonroot_storage(
    ...
    def accept_install(service_id):
        assert service_id == "silverbullet"
        installed = users / service_id
        service = yaml.safe_load((installed / "compose.yaml").read_text())["services"][service_id]
        assert service["command"] == yaml.safe_load((library / "silverbullet/compose.yaml").read_text())["services"]["silverbullet"]["command"]
        assert service["environment"]["SB_SHELL_BACKEND"] == "off"
        assert service["user"] == "1000:1000"
        _mod._precreate_data_dirs(service_id)
        assert (tmp_path / "data/silverbullet").is_dir()
        ownership.assert_any_call(str(tmp_path / "data/silverbullet"), 1000, 1000)
        return True
    ...
    try:
        ...
        response = test_client.post("/api/extensions/silverbullet/install", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
        install.assert_called_once_with("silverbullet")
    finally:
        server.shutdown()
        server.server_close()
