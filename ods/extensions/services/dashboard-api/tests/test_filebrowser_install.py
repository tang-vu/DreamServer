"""The authenticated install must materialize the file workspace definition before host startup."""
...
def test_filebrowser_http_install_preserves_authentication_and_prepares_nonroot_storage(
    ...
    def accept_install(service_id):
        assert service_id == "filebrowser"
        installed = users / service_id
        service = yaml.safe_load((installed / "compose.yaml").read_text())["services"][service_id]
        assert service["command"] == yaml.safe_load((library / "filebrowser/compose.yaml").read_text())["services"]["filebrowser"]["command"]
        assert service["environment"]["FB_DISABLE_EXEC"] == "true"
        assert service["user"] == "1000:1000"
        _mod._precreate_data_dirs(service_id)
        for name in ("files", "database", "config"):
            path = tmp_path / "data/filebrowser" / name
            assert path.is_dir()
            ownership.assert_any_call(str(path), 1000, 1000)
        return True
    ...
    try:
        ...
        response = test_client.post("/api/extensions/filebrowser/install", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
        install.assert_called_once_with("filebrowser")
    finally:
        server.shutdown()
        server.server_close()
