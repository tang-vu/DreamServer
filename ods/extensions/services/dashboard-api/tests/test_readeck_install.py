"""The authenticated install must materialize the reading library definition before host startup."""
...
def test_readeck_http_install_preserves_authentication_and_prepares_nonroot_storage(
    ...
    def accept_install(service_id):
        assert service_id == "readeck"
        installed = users / service_id
        service = yaml.safe_load((installed / "compose.yaml").read_text())["services"][service_id]
        assert service["command"] == yaml.safe_load((library / "readeck/compose.yaml").read_text())["services"]["readeck"]["command"]
        assert service["environment"]["READECK_TRUSTED_PROXIES"] == "127.0.0.1/32,::1/128"
        assert service["user"] == "1000:1000"
        _mod._precreate_data_dirs(service_id)
        assert (tmp_path / "data/readeck").is_dir()
        ownership.assert_any_call(str(tmp_path / "data/readeck"), 1000, 1000)
        return True
    ...
    try:
        ...
        response = test_client.post("/api/extensions/readeck/install", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
        install.assert_called_once_with("readeck")
    finally:
        server.shutdown()
        server.server_close()
