"""The authenticated install must materialize the CouchDB configuration before host startup."""
...
def test_couchdb_http_install_syncs_config_and_prepares_nonroot_storage(
    ...
    def accept_install(service_id):
        assert service_id == "couchdb"
        installed = users / service_id
        service = yaml.safe_load((installed / "compose.yaml").read_text())["services"][service_id]
        mount = next(item for item in service["volumes"]
                     if isinstance(item, dict) and item["target"] == "/opt/couchdb/etc/local.ini")
        target = tmp_path / mount["source"]
        assert target.is_file()
        assert target.read_bytes() == (installed / "config/couchdb/local.ini").read_bytes()
        assert mount["read_only"] is True
        assert service["user"] == "5984:5984"
        _mod._precreate_data_dirs(service_id)
        for name in ("databases", "config"):
            assert (tmp_path / "data/couchdb" / name).is_dir()
            ownership.assert_any_call(str(tmp_path / "data/couchdb" / name), 5984, 5984)
        return True
    ...
    try:
        ...
        response = test_client.post("/api/extensions/couchdb/install", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
        install.assert_called_once_with("couchdb")
    finally:
        server.shutdown()
        server.server_close()
