"""The authenticated install must materialize the status history guard before host startup."""
...
def test_gatus_http_install_syncs_guard_and_prepares_nonroot_storage(
    ...
    def accept_install(service_id):
        ...
        mount = next(item for item in service["volumes"]
                     if isinstance(item, dict) and item["target"] == "/config/config.yaml")
        target = tmp_path / mount["source"]
        assert target.is_file()
        assert target.read_bytes() == (installed / "config/gatus/config.yaml").read_bytes()
        assert mount["read_only"] is True
        assert service["user"] == "1000:1000"
        _mod._precreate_data_dirs(service_id)
        assert (tmp_path / "data/gatus").is_dir()
        ownership.assert_any_call(str(tmp_path / "data/gatus"), 1000, 1000)
        return True
    ...
    try:
        ...
        response = test_client.post("/api/extensions/gatus/install", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
        install.assert_called_once_with("gatus")
    finally:
        server.shutdown()
        server.server_close()
