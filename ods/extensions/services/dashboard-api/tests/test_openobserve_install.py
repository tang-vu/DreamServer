"""The authenticated install must materialize the observability definition before host startup."""
...
def test_openobserve_http_install_preserves_authentication_and_prepares_nonroot_storage(
    ...
    def accept_install(service_id):
        assert service_id == "openobserve"
        installed = users / service_id
        service = yaml.safe_load((installed / "compose.yaml").read_text())["services"][service_id]
        assert service["image"] == yaml.safe_load((library / "openobserve/compose.yaml").read_text())["services"]["openobserve"]["image"]
        assert service["environment"]["ZO_TELEMETRY"] == "false"
        assert service["user"] == "10001:10001"
        _mod._precreate_data_dirs(service_id)
        assert (tmp_path / "data/openobserve").is_dir()
        ownership.assert_any_call(str(tmp_path / "data/openobserve"), 10001, 10001)
        return True
    ...
    try:
        ...
        response = test_client.post("/api/extensions/openobserve/install", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
        install.assert_called_once_with("openobserve")
    finally:
        server.shutdown()
        server.server_close()
